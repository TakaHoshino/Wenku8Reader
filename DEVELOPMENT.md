...
- **定位**：**单书架是正式功能，多书架是实验功能**。开关 `ReaderSettingsState.multiShelfEnabled`
  （DataStore 键 `multi_shelf_enabled`，**默认 false**）位于设置 → 实验性，Material 与 MIUIX 各一个。
  关闭时书架页与收藏路径与单书架版本逐像素一致（连弹窗都不出现，收藏/取消收藏仍是点一下即刻生效），
  且**不删任何数据**，重新开启即恢复。多书架相关的改动不要带进 master 的正式发布，除非明确要转正。
