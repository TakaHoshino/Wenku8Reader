...
另有一套 **MIUIX（HyperOS 风格）** 界面（实验性，默认关闭）：页面、弹层、设置项全部换成 miuix 组件，含悬浮胶囊底栏与液态玻璃背景，可在「设置 → 实验性」整体切换。

> 关于「实验性」：这类功能**默认关闭**，关闭时应用行为与正式版完全一致，在「设置 → 实验性」里逐项开启。
> 单书架是对外承诺的正式行为，**多书架与账户登录是实验性功能**。

## 功能特性

-  **探索发现**：首页栏目（封面轮播 + 文字榜单）、书名/作者搜索、分类标签浏览（可按标签分页加载全部）
-  **我的书架**：收藏管理、多种排序（默认/最近更新/书名/字数/倒序）、已读章节进度、一键继续阅读
-  **多书架分组**（实验性，默认关闭）：新建/重命名/删除书架，一本书可同时属于多个书架；收藏时勾选目标书架，长按卡片改归属，书架页顶部一键切换
-  **账户登录**（实验性，默认关闭，依赖多书架）：登录自己的 wenku8 账户后，书架里多出与网站**双向同步**的「Wenku8书架」——在收藏弹窗里勾选即站方加入、取消勾选即站方移出；退出登录立刻回落内置账号，阅读无感续用（**不保存密码**）
-  **阅读统计**：GitHub 风格阅读热力图（本周/本月/本年/全部）+ 累计时长/连续天数/日均汇总，点日期查看当天明细
-  **沉浸式阅读器**：
  - 侧滑翻页 / 滚动翻页双模式，点按左右翻页
  - 自动翻页（可设间隔）、音量键翻页
  - 章节进度自动记录，重开自动续读
  - 独立目录页：分卷折叠、已读标记，重读时自动重置该章已读状态
  - 简体 ↔ 繁体一键切换
...
-  **应用内更新**：正式版 / 测试版双通道检查与下载，**安装前校验 APK 签名**（与已安装应用不一致则拒绝安装）
-  **存储管理**：按类型统计与清理（图片缓存 / 更新包 / WebView / 网页缓存）、过期阅读记录清理、缓存上限
-  **个性化外观**：动态取色 / 手动种子色、深色模式（跟随系统/浅色/深色）、OLED 纯黑模式、触觉反馈强度
...
| 书架 | 卡片化书单，进度一目了然，一键续读；开启多书架后顶部出现书架切换条与「管理书架」入口 |
| 详情 | 封面信息卡、标签药丸、开始/继续阅读、离线下载卡、内容简介；作者与标签可点击跳转 |
| 阅读器 | 沉浸式全屏，状态指示器（电量/时间/章节/进度），悬浮章节进度条 |
| 统计 | 阅读热力图 + 汇总卡 + 当日明细（书架顶栏日历图标进入） |
| 设置 | 分类入口二级页：账号、外观（主题/纯黑/取色）、存储、实验性（界面风格 / 多书架 / 账户登录）、网络、更新、阅读设置、关于 |
...
- **安装包**：GitHub Releases 发布（自动化：推送 `master` 后依据 Conventional Commits 自动升级版本、构建并发布；`dev` 分支每次推送产出测试版预发布包。见 [VERSIONING.md](VERSIONING.md)）
...
- **开发文档**：架构、存储与实现细节见 [DEVELOPMENT.md](DEVELOPMENT.md)；版本历史见 [CHANGELOG.md](CHANGELOG.md)；提交与分支约定见 [CONTRIBUTING.md](CONTRIBUTING.md)
...
原生 **Kotlin + Jetpack Compose** 构建，无 WebView 外壳，界面有 **Material 3 Expressive**（默认）与 **MIUIX / HyperOS**（实验性）两套独立实现；网络层基于 OkHttp + Cronet，图片加载使用 Coil，简繁转换使用 opencc4j，书架与阅读进度用 Room、设置用 DataStore 持久化。
